module Accessors
  ...
  module ClassMethods
    def attr_accessor_with_history(*attributes)        
      attributes.each do |attribute|
        ...
        define_method("#{attribute}=") do |value|                   
          ...
          #@attributes_history ||= {}
          # with unknown instance variable for #method_missing. For example :driver
          @attributes_history ||= { :driver => %w[Alex Peter Jack] }
    ...
    def strong_attr_accessor(attribute, type)
      ...
      #setter
      define_method("#{attribute}=") do |value|
        raise TypeError.new("Значение не является типом #{type.to_s}") unless value.instance_of?(type)
        instance_variable_set(attr_name, value)
      rescue TypeError => e
        puts e.message
      end    
  ...
  module InstanceMethods
    def method_missing(name)
      instance_var = name.to_s.split('_')[0].to_sym
      @attributes_history[instance_var]
    end
  end  
end
